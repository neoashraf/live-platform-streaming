                        ...
                        return cachePort.getLiveRoomById(giftTransaction.getLiveRoomId())
                                        ...
                                        .map(liveRoomFirebaseEntity -> {
                                                ...
                                                if (giftTransaction.getSenderReceiverDto().getReceiver().getUserType()
                                                        ...
                                                        double giftsReceivedInThisSessionAsHost = liveRoomFirebaseEntity
                                                                        .getHost().getGiftsReceivedInThisSession();

                                                        liveRoomFirebaseEntity.getHost().setGiftsReceivedInThisSession(
                                                                        giftsReceivedInThisSessionAsHost
                                                                                        + giftTransaction.getBeans());
                                                } else {
                                                        List<JoinRequests> joinRequests = liveRoomFirebaseEntity
                                                                        .getJoinRequests();
                                                        // System.out.println("\n\n join requests status:
                                                        // "+joinRequests.stream().map(JoinRequests::getStatus)+"
                                                        // \n\n");
                                                        String receiverId = giftTransaction.getReceiverId();
                                                        Optional.ofNullable(this.receiverValidation(joinRequests,
                                                                        receiverId))
                                                                        .ifPresent(joinRequests2 -> {
                                                                                joinRequests2.setGiftsReceivedInThisSession(
                                                                                                joinRequests2.getGiftsReceivedInThisSession()
                                                                                                                + giftTransaction
                                                                                                                                .getBeans());
                                                                        });
                                                }
